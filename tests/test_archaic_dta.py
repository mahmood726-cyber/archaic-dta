...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("archaic_dta_simulation", script_path)
...
def test_main_writes_relative_outputs(tmp_path):
    ...
    assert certification["project"] == "archaic-dta"
    assert certification["locator"] == "data.csv"
    assert result["dataframe"].shape[0] == 15
